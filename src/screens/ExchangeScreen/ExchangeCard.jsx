...
const API =
  'https://api.coingecko.com/api/v3/simple/price?ids=bitcoin%2Cethereum&vs_currencies=usd%2Ceur%2Cvnd&include_market_cap=false&include_24hr_vol=false&include_24hr_change=false&include_last_updated_at=false&precision=4';
...
const ExchangeCard = () => {
  ...
  return (
    <View style={styles.container}>
      <Switch isLeft={isBuy} setLeft={setBuy} />
      <View style={styles.body}>
        <View style={styles.conversionRate}>
          {/* hien thi */}
          <Text style={{ color: '#CACACA', fontSize: 16, marginRight: 10 }}>
            1 {coinSelected} = {priceExchange} {currencySelected}
          </Text>
          <TouchableOpacity>
        ...
        <View style={styles.input}>
          ...
          >
            <LinearGradient colors={['#FF2CDF', '#8020EF', '#0014FF']} start={{ x: 0, y: 1 }} end={{ x: 1, y: 1 }} style={styles.exchangeButton}>
              <Text style={styles.exchangeText}>Chuyển đổi</Text>
            </LinearGradient>
          </TouchableOpacity>

          {coinListOpen && <DropDownList style={{ marginTop: 21 }} data={coinList} show={coinListOpen} setShow={setCoinListOpen} setSelected={setCoinSelected} />}

          {currencyListOpen && (
            <DropDownList style={{ marginTop: 89 }} data={currencyList} show={currencyListOpen} setShow={setCurrencyListOpen} setSelected={setCurrencySelected} />
          )}
        </View>
